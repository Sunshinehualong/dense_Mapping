...
private:
     ...
     ros::Subscriber transform_sub_, depth_sub_; //订阅位姿和深度信息
     ros::Timer update_mesh_timer_;              //ros定时器　更新mesh

     Eigen::Vector3d sync_pos_, raycast_origin_, cur_pos_;
     Eigen::Quaterniond sync_q_;

     std::queue<std::tuple<ros::Time, Eigen::Vector3d, Eigen::Quaterniond>> transform_queue_;//位姿的buffer 时间戳＋位置和旋转
     std::queue<DepthMsgType> depth_queue_;//深度信息的buffer 点云or depth image
...
#else
     std::vector<int> set_free_, set_occ_; // free or occupied
#endif
...
void Fiesta<DepthMsgType, PoseMsgType>::PoseCallback(const PoseMsgType &msg)
{
     ...
     transform_queue_.push(std::make_tuple(msg->header.stamp, pos, q)); //把时间戳　位置和旋转放到buffer中
}
