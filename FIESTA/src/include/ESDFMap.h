...
#ifdef PROBABILISTIC
  std::vector<double> occupancy_buffer_;  // 0 is free, 1 is occupied    记录每一个voxels是否被占用的情况
#else
  std::vector<unsigned char> occupancy_buffer_;  // 0 is free, 1 is occupied
#endif
  std::vector<double> distance_buffer_;
  std::vector<int> num_hit_, num_miss_; //每个voxels　hit miss的数量
  std::vector<Eigen::Vector3i> closest_obstacle_;//记录每一个voxels最邻近障碍物的点的voxel位置
  std::vector<int> head_, prev_, next_;//记录每一个voxels的head prev next的索引
...
// Map Properties
  ...
  int total_time_ = 0;
  int infinity_, undefined_;// 无穷远和未定义的点，10000  -10000
